...
| Stage 3 | #56 | STG3-B-2 (`/payment/transfer` → MoneyMovement, first live wiring) | Open — stacked on #49, #51, #54 |
| Stage 3 | #57 | DEF-STG3-WALLET-WORKER (compensation queue drain worker) | Open — stacked on #51 |
...
| ~~DEF-STG3-B-2~~ | ~~Migrate `/payment/transfer` to `moneyMovement.process(intent)`~~ | — | **Shipped in PR #56** (stacked on #49, #51, #54) |
...
| ~~DEF-STG3-WALLET-WORKER~~ | ~~Compensation queue drain worker~~ | — | **Shipped in PR #57** (stacked on #51). Cron wiring + admin endpoint follow in next PR. |
| DEF-STG3-WORKER-CRON | Wire wallet compensation worker into `recurringScheduler.ts` + add admin GET endpoint for queue view | #57 merged | 1d (cron + endpoint + integration test) |
...
| DEF-LOW-OBSERVABILITY | pino → Honeycomb + OpenTelemetry trace IDs across every money-out call | Stage 4 item 14. PR #56 unblocks this — the MoneyMovement service is the natural span boundary. Add OTel auto-instrumentation in a follow-up sprint. |
...
| MEDIUM | ~6 | 1 (typed approval cols) | 1 (transactions backfill) | 3 (Stage-3 follow-ups; was 4 — DEF-STG3-B-2 shipped in #56) | 4 |
| LOW | ~3 | 0 | 0 | 0 | 3 |
